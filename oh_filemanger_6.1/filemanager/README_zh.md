...
1. **存储位置管理**：支持浏览和管理多个存储位置，包括我的手机（内部存储），外置存储卡（U盘，SD卡），收藏文件夹及最近删除（回收站）；支持文管看图库，快速浏览图片和视频文件。
...
4. **整理文件操作**
   ：提供文件管理操作能力：支持多选，打开，其他应用打开，新建目录，重命名，复制，粘贴，删除（移入最近删除），彻底删除，还原（从最近删除处恢复），移动，收藏/取消收藏；支持打印文件，将图片设为壁纸，将铃声设为铃音；支持清空回收站。
